...
?>
	<?if(isset($arResult['MESSAGE'])): CJSCore::Init(array("popup"));?>
			...
			<div class="bx_subscribe_response_container">
				<table>
					<tr>
						...
						<td>
							<div class="subsc<?=$arResult['MESSAGE']['TYPE']?>" style="font-size: 22px;"><?=GetMessage('subscr_form_response_'.$arResult['MESSAGE']['TYPE'])?></div>
		...
		<script>
			ym(31407293, 'reachGoal', 'leave_an_email');
			gtag('event','leave_an_email',{'event_category':'leave_an_email','event_label':'Lavender'});
		</script>
		<script>
			BX.ready(function(){
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>" class="subscribe__form">
		<?=bitrix_sessid_post()?>
		<input type="hidden" name="sender_subscription" value="add">

		<input type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="Ваш e-mail">
		...
		<div class="bx_subscribe_submit_container">
			<button class="btn" id="bx_subscribe_btn_<?=$buttonId?>"><span>Подписаться</span></button>
		</div>
	</form>
<?
$frame->beginStub();

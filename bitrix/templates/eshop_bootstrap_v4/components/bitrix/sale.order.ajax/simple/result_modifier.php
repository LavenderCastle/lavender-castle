 ...
 */
 ...
 foreach ($arResult['BASKET_ITEMS'] as $basketItem):
   if (!str_contains($basketItem['DETAIL_PAGE_URL'], 'price-opt-suhotsvety')) {
     $isOpt = false;
   }
 ...
 if ($arResult['DELIVERY'][7]['CHECKED']='Y' || $arResult['DELIVERY'][8]['CHECKED']='Y'){
   $paysys = 3;
 } else {
   $paysys = 2;
 }
 if ($isOpt){
   ?>
   <style>
      #bx-soa-paysystem .bx-soa-pp-company:nth-child(<?=$paysys?>) {
        display: none;
      }
   </style>
   <script>
     document.addEventListener("DOMContentLoaded", function() {
       if ($('#ID_PAY_SYSTEM_ID_8').prop('checked')){
         $('#ID_PAY_SYSTEM_ID_16').prop('checked',true);
       }
     });
   </script>
   <?
   ...
   ?>
   <style>
      #bx-soa-paysystem .bx-soa-pp-company:nth-child(<?=$paysys+1?>) {
        display: none;
      }
   </style>
   <?

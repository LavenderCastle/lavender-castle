...
document.addEventListener("DOMContentLoaded", function(event) {
	...
	for ( var i = 0; i < sliders.length; i++ ) {
	    $('.filter__inputs').eq(i).find('input').attr('data-index',i);
	    minVal = parseInt($('.filter__inputs').eq(i).find('.min-price').val());
	    maxVal = parseInt($('.filter__inputs').eq(i).find('.max-price').val());
	...
	function changeInputs(i){
	  for (var i = 0; i < sliders.length; i++) {
	    $('.filter__inputs').eq(i).find('.min-price').val(parseInt(sliders[i].noUiSlider.get()[0]));
	    $('.filter__inputs').eq(i).find('.max-price').val(parseInt(sliders[i].noUiSlider.get()[1]));
	  }
	}
	...
	$('.min-price').change(function(){
	  ...
	  var max = parseInt($(this).parent().parent().parent().find('.max-price').val());
	  if (val > max) {
	    $(this).val(max);
	  } else {
	...
	$('.max-price').change(function(){
	  var i = $(this).data('index');
	  var val = parseInt($(this).val());
	  var min = parseInt($(this).parent().parent().parent().find('.min-price').val());
	  if (val < min) {
	    $(this).val(min);
	  } else {
